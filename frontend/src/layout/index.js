        ...
        if (data && data.length > 0) {
          ...
          if (lightConfig) {
            setCompanyData((prevData) => ({
              ...prevData,
              name: lightConfig.company || "NP DEV®",
              url: lightConfig.url || "https://npdev.com.br",
            }));
          }
        }
      } catch (err) {
    ...
    const fetchLogo = async () => {
      try {
        ...
        if (data && data.length > 0) {
          ...
          const darkConfig = data.find(
            (themeConfig) => themeConfig.theme === "dark",
          );
      ...
      const fetchDrawerState = async () => {
        try {
          ...
          if (err.response && err.response.status === 403) {
            toastError(
              "Acesso negado. Você não tem permissão para acessar esta configuração.",
            );
          } else {
            setDrawerOpen(true);
            toastError(err);
        ...
        <StyledToolbar variant="dense">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <Tooltip
              title={
                isFullscreen
                  ? t("mainDrawer.appBar.fullscreen.exit")
                  : t("mainDrawer.appBar.fullscreen.enter")
              }
            >
              <IconButton
            ...
            >
              <MenuItem
                ...
                sx={{ py: 1.5, px: 2.5, borderRadius: 1, mx: 0.5, my: 0.5 }}
              >
