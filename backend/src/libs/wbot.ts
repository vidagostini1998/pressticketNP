  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!process.env.COMPANY_NAME) {
        clientSession = `session-${whatsapp.id}`;
      }
      ...
      // Adiciona lógica de retry para inicialização do wbot
      let attempts = 0;
      const maxAttempts = 3;
      while (attempts < maxAttempts) {
        try {
          ...
          attempts++;
          logger.error(`Erro ao inicializar wbot (tentativa ${attempts}): ${err.message}`);
